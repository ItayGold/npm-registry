...
export class ClickTreeNodeComponent implements OnInit {
  ...
  @HostBinding('class') class = 'tree-item-node';
  ...
  onNodeClick($event: Event, node: ClickTreeNode) {
  ...
  onChecked(nextState: number, node: ClickTreeNode) {
  ...
  private setCheckState(keyNode: ClickKeyNode, state: CheckboxState) {
  ...
  private setParentState(keyNode: ClickKeyNode) {
    const node = this.nodes.get(keyNode.key);
    if (!node.parentKey) {
      return;
    }
